...
local function kfk_destroy_buf(kfk, kfk_buf, err)
    ...
    if err == 0 then
        kfk.succ_msg_cnt = kfk.succ_msg_cnt + msgq.size;
        kfk.msg_cnt = kfk.msg_cnt - msgq.size;
        list.init(kfk_buf.msgq);
        return;
    end
...
local function kfk_broker_new(kfk, host, port, nodeid)
	return {
...
local function kfk_broker_startup(kfk_broker)
    local kfk = kfk_broker.kfk;
...
local function kfk_broker_add(kfk, startup, node)
	local brokers = kfk.kfk_brokers;
	local kfk_broker = kfk_broker_new(kfk, node.host, node.port, node.nodeid);
	
    if startup then
        kfk_broker_startup(kfk_broker);
    else
...
local function kfk_broker_adds(kfk, brokers)
	...
	for _, broker in ipairs(brokers) do
		local name = broker.host .. ":" .. broker.port;
        local kfk_broker = kfk_broker_find_byname(kfk, name);
        if not kfk_broker then
            if kfk_broker_add(kfk, true, broker) then
			    cnt = cnt + 1;
            end
        elseif kfk_broker.state == const.KFK_BROKER_DOWN then
            if kfk_broker_startup(kfk_broker) then
                cnt = cnt + 1;
            end
		end
	end
